...
const methods = {
    ...
    async serializeAbilities (pokeAbility) {
        ...
        return [capablePokemons.id,capablePokemons.name, {effect: capablePokemons.effect_entries[1].effect,capable}]
    },
...
export async function getDataList() {
    const pokemons = await get('https://pokeapi.co/api/v2/pokemon?limit=100000&offset=0')
    const abilities = await get('https://pokeapi.co/api/v2/ability?limit=100000&offset=0')
    return {pokemons: pokemons.results, abilities: abilities.results}
}